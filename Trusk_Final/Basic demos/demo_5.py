...
def get_z(triangle):
    return (triangle.p[0].z + triangle.p[1].z + triangle.p[2].z)
...
def get_object_from_file(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            #print("import " + str((n/total) *100) + "%", end = " ")
            string = str(round((n/total) *100, 0)) + "%" 
            sys.stdout.write("\r[" + "#" * int(round((n/total) *100, 0)) + "_" * int(round(100 - (n/total) *100, 0)) + "] ")
...
meshCube = get_object_from_file('resources/sample_17.obj')
matProj = matrix4x4()
...
def update(r, fTheta):
    ...
    for tri in meshCube.triangles:
        ...
        triTranslated.p[0].z = triRotatedZX.p[0].z - 1.1**zoom - 1
        triTranslated.p[1].z = triRotatedZX.p[1].z - 1.1**zoom - 1
        triTranslated.p[2].z = triRotatedZX.p[2].z - 1.1**zoom - 1
    ...
    vecTrianglesToRaster.sort(key=get_z)
        
    for t in vecTrianglesToRaster:
        r.filled_triangle(t.p[0].x, t.p[0].y, t.p[1].x, t.p[1].y, t.p[2].x, t.p[2].y, color = adjust_lightness("#34ebc3", (t.col+1)/2.5))
        #r.triangle(t.p[0].x, t.p[0].y, t.p[1].x, t.p[1].y, t.p[2].x, t.p[2].y, color = adjust_lightness("#000000", (t.col+1)/2.5), width = 1)
