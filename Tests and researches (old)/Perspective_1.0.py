...
player = [[0],[0],[0]]
#todo: move around player, rotate around player
light = [[0],[1],[0]]
...
def connect_triangles(i, j, k, points, r):
    direction = np.cross([points[j][0] - points[i][0], points[j][1] - points[i][1]], [points[k][0] - points[i][0], points[k][1] - points[i][1]])
    norm = direction
    
    lighting = np.dot(light, norm)
    print(str(lighting[1][0]/50000 + 0.5))
    l = lighting[1]/60000 + 0.5
    r.filled_triangle(points[i][0], points[i][1], points[j][0], points[j][1], points[k][0], points[k][1], color = str(lighting[1][0]/60000 + 0.5))
...
def render(r, angle):
    ...
    translation_z = translation_matrix = [
                    [angle, 0, 0],
                    [0, angle, 0],
                    [0, 0, angle],]
    
    translation_x = translation_matrix = [
                    [1, 0, 0],
                    [0, 1, 0],
                    [0, 0, 1],]
    ...
    for point in points:
        ...
        rotated_2d = matrix_multiplication(translation_x, rotated_2d)
        distance = 5
